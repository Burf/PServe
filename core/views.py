...
from burf.process.multiworkspace import MultiWorkspace
                 ...
                 "/get_result/":{"context":"Get result for executed job",
                                 "json":{"requset":{"name":"Workspace name(Necessary field)",
                                         ...
                                         "response":{"request_result":"Request result",
                                                     "result":"Executed job result"}}}}
VOL = Volume()
STORE = VOL.create_volume("__PSERVE__")
            ...
            if job_num is not None:
                ...
                while response_time < vo.timeout:
                    ...
                    if job_finish:
                        ...
                        variable = VariableEncoder(MW.get_variable(vo.name), encoder_func)
                        result = {"job":job_num, "finish":job_finish, "result":job_result, "time":response_time, "variable":variable}
                        break
                    else:
...
def get_result(request):
    if request.method == "POST":
        try:
            vo = ValueObject(json.loads(request.body))
            result = MW.get_result(vo.name, vo.job)
            request_result = result is not None
            response_json = {"request_result":request_result, "result":result}
        except Exception as e:
            print(e)
